...
| ~~DATA-3~~ (partiel) | Corpus structuré + provenance | `source_id` sur tous les modèles, `verifier_provenance()` garantit qu'aucune référence n'est orpheline (testé sur les données réelles). **Incomplet** : les débouchés professionnels ne sont pas collectés — le site officiel de l'ISPM ne les publie pas | SETUP-5, DATA-2 |
...
| ~~DATA-5~~ ✅ | Registre de collecte de l'enquête | `backend/src/enquete.py` (`RegistreCollecte`) + `backend/data/enquete/registre_collecte.json` — tous les champs exigés au §5, chiffres vérifiés contre les données réelles par test, et limites nommées (volume, couverture des parcours, population professionnelle quasi absente) | DATA-4 |
