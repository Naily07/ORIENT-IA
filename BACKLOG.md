...
| ~~DATA-3~~ ✅ | Corpus structuré + provenance | `source_id` sur tous les modèles, `verifier_provenance()` garantit qu'aucune référence n'est orpheline. **Complété** : 66 métiers et les débouchés des 16 parcours importés depuis une source tierce (SRC-EXTERNE), après recoupement de ses prérequis avec notre source officielle (14 concordances sur 16). Le graphe passe de 27 à 583 nœuds, et `identifier_debouches` renvoie enfin de vraies données | SETUP-5, DATA-2 |
