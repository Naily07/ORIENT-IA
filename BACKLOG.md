...
| ~~ORCH-5~~ ✅ | Le budget de temps ne borne pas la durée totale | **Défaut observé en testant l'interface dans un navigateur réel** : une demande a mis 208 s côté serveur, dépassant le délai du client (180 s) — l'utilisateur voyait une erreur réseau alors que le backend avait correctement dégradé et produit une réponse valide. **Corrigé** : `traiter_demande()` installe une échéance isolée par requête ; `llm_client` borne désormais avec le temps restant le verrou et l'attente de lissage, le timeout HTTP de chaque tentative et les pauses de reprise. Une étape LLM déjà lancée ne peut donc plus consommer son ancien maximum d'environ 140 s au-delà du budget global de 120 s | ORCH-3 |
