...
| ORCH-5 | Le budget de temps ne borne pas la durée totale | **Défaut observé en testant l'interface dans un navigateur réel** : une demande a mis 208 s côté serveur, dépassant le délai du client (180 s) — l'utilisateur voit une erreur réseau alors que le backend avait correctement dégradé et produit une réponse valide. Cause : `orchestrator._budget_epuise()` est vérifié **avant** de lancer une étape, mais n'interrompt pas une étape déjà partie ; or un appel LLM peut durer jusqu'à ~140 s à lui seul (`llm_max_tentatives` 4 × `llm_timeout_s` 30 s + reprises de 6,5 s). Deux appels successifs dépassent donc le budget de 120 s. **Piste** : borner la durée cumulée en passant le temps restant à `llm_client` (réduire le timeout et le nombre de tentatives au fil du budget), plutôt que de vérifier seulement entre les étapes | ORCH-3 |
...
| ~~SEC-5~~ ✅ | Mention obligatoire dans l'interface | « ORIENT'IA... ne remplace ni l'avis d'un conseiller pédagogique ni une décision officielle d'admission » **[NOUVEAU]** — texte figé dans `config.MENTION_OBLIGATOIRE`, exposé par `GET /health` et **affiché à deux endroits** dans l'interface (sidebar dépliée d'office + zone principale, en tête de chaque page). Le doublon est délibéré : vérifié dans un navigateur réel, la sidebar n'apparaissait pas au premier chargement, donc la mention non plus — le §16 exige de l'afficher *clairement* | FE-1 |
...
| ID | Tâche | Description | Dépendances |
|---|---|---|---|
| ~~FE-1~~ ✅ | Squelette Streamlit | Chat, scénarios pré-remplis, onglet observabilité, explorateur de données **[REUSE]** `frontend/app.py` comme base, vocabulaire ticket → profil/recommandation — trois pages (Orientation, Observabilité, Corpus et graphe), état de l'API en sidebar, 5 scénarios de démonstration pré-remplis, gestion d'erreur réseau explicite. Débloque au passage `run.sh`/`run.ps1`, qui référençaient `frontend/app.py` depuis le premier jour sans que le fichier existe | ORCH-4 |
| ~~FE-2~~ ✅ | Collecte progressive du profil | Formulaire ou chat multi-tours pour construire le profil candidat **[NOUVEAU]**, spécifique à l'exigence « recueillir progressivement le profil » — formulaire à champs tous facultatifs, saisie libre séparée par des virgules. Les termes ne sont **volontairement pas normalisés côté interface** : `src.ml.vocabulaire` (ML-9) les résout et signale ce qu'il n'a pas reconnu ; nettoyer ici masquerait ce diagnostic | SETUP-4 |
| ~~FE-3~~ ✅ | Affichage de la recommandation | Parcours proposés, score, sources citées, incertitude déclarée **[ADAPT]** carte de décision d'EXAM-S2 — badge d'action lisible (information / recommandation / demande d'informations / conseiller / administration), avertissement visible quand `incertitude_declaree`, sources et outils côte à côte, confiance et `trace_id` rappelés, JSON brut dépliable. Les scores sont affichés **tels que produits par le modèle**, jamais recalculés par l'interface | FE-1, ORCH-1 |
| ~~FE-4~~ ✅ | Onglet explorateur de graphe | Visualisation des relations Compétence/Parcours/Métier, si ontologie construite **[NOUVEAU]** — graphe rendu en DOT via `st.graphviz_chart` (aucune dépendance de tracé ajoutée), avec une légende honnête : les relations `enseigne`/`developpe`/`prepareA` sont annoncées comme absentes tant que matières, compétences et débouchés ne sont pas collectés (DATA-1) | ONTO-2 |
