...
| ID | Tâche | Description | Dépendances |
|---|---|---|---|
| ~~DATA-1~~ (partiel) | Collecte du corpus ISPM | Mentions, parcours, niveaux, prérequis depuis `ispm-edu.com` + recoupement annuaire.mg **[NOUVEAU]** — 6 mentions et 16 parcours réels. **Matières collectées** (27/08) : 490 intitulés rattachés aux 16 parcours, extraits par OCR de 129 calendriers d'épreuves (L1→M2, S1/S2) transmis par l'équipe — `backend/scripts/extraire_matieres.py`, mécanisme reproductible et testé. **Source externe** (groupe Facebook étudiant) malgré l'apparence officielle des documents : limites enregistrées au registre (`SRC-CALENDRIERS-FACEBOOK`). Le rattachement suit la **classe imprimée** sur le document et non le nom de dossier : les calendriers de master rangés sous « TEH » annoncent « Classe : TOUR4/TOUR5 », classe qui réunit les deux parcours de la mention Tourisme — sans quoi TEE se retrouvait à zéro matière. **Restent incomplets** : compétences, débouchés métiers, passerelles, et les matières de licence propres à TEE. **Débouchés (28/08)** : 66 métiers candidats extraits d'une source externe générée automatiquement (non ISPM, registre `SRC-GUIDE-FILIERES-GENERE`) via `backend/scripts/preparer_candidats_metiers.py` → `backend/data/a_valider/metiers_candidats.json`, en attente de relecture humaine avant fusion dans `metiers.json`/`parcours.json` (règle §4, pas de fusion automatique d'une source non vérifiée) | — |
| ~~DATA-2~~ ✅ | Registre des sources | Structure (titre, URL, date de consultation, statut officiel/institutionnel/externe, données extraites, limites) — absent d'EXAM-S2 **[NOUVEAU]** — `backend/src/sources.py` + `backend/data/registre_sources.json` (7 entrées, dont une divergence de source explicitement documentée sur le sigle CAA, une expansion non confirmée pour AEE, et une source externe générée automatiquement pour les débouchés candidats) | DATA-1 |
| ~~DATA-3~~ (partiel) | Corpus structuré + provenance | Peupler `DocumentSource`/`Mention`/`Parcours` (SETUP-5) en liant chaque entrée à son entrée du registre (DATA-2) **[ADAPT]** ingestion RAG-1 d'EXAM-S2 — `source_id` posé sur tous les modèles, `verifier_provenance()` garantit qu'aucune référence n'est orpheline (testé sur les données réelles). **Incomplet** : matières/compétences/métiers/passerelles pas encore peuplés dans le corpus de production (dépend de DATA-1) — des candidats de métiers existent hors production, voir note DATA-1 | SETUP-5, DATA-2 |
| ~~DATA-4~~ (partiel) | Questionnaire d'enquête | Rédaction et diffusion (étudiants + professionnels), lancé dès la première heure du hackathon **[NOUVEAU]** — **rédigé** : `backend/data/enquete/questionnaire.md` (version de référence, 21 questions, deux populations, consentement explicite, aucune donnée sensible) et `generer_google_form.gs`, script Apps Script qui génère le formulaire Google à l'identique depuis cette référence. **Diffusée** : 86 réponses reçues (voir DATA-5) | — |
| ~~DATA-5~~ ✅ | Registre de collecte de l'enquête | Populations visées, période, nombre de réponses reçues/retenues/écartées, texte de consentement, procédure d'anonymisation, biais constatés **[NOUVEAU]** — `backend/data/enquete/registre_collecte.md`, généré automatiquement par `backend/scripts/preparer_jeu_test_reel.py` : 86 réponses (71 étudiants, 15 professionnels), 79/86 rattachables à un parcours ou une mention connue, 62/86 à étiquette jugée fiable (satisfaction ≥ 3/5). Biais et limites (échantillon concentré sur quelques mentions, risque de ré-identification par petits effectifs) documentés dans le fichier généré, pas seulement déclarés ici | DATA-4 |
| ~~DATA-1~~ (partiel) | Collecte du corpus ISPM | Mentions, parcours, niveaux, prérequis depuis `ispm-edu.com` + recoupement annuaire.mg **[NOUVEAU]** — 6 mentions et 16 parcours réels. **Matières collectées** (27/08) : 490 intitulés rattachés aux 16 parcours, extraits par OCR de 129 calendriers d'épreuves (L1→M2, S1/S2) transmis par l'équipe — `backend/scripts/extraire_matieres.py`, mécanisme reproductible et testé. **Source externe** (groupe Facebook étudiant) malgré l'apparence officielle des documents : limites enregistrées au registre (`SRC-CALENDRIERS-FACEBOOK`). Le rattachement suit la **classe imprimée** sur le document et non le nom de dossier : les calendriers de master rangés sous « TEH » annoncent « Classe : TOUR4/TOUR5 », classe qui réunit les deux parcours de la mention Tourisme — sans quoi TEE se retrouvait à zéro matière. **Restent incomplets** : compétences, débouchés métiers, passerelles, et les matières de licence propres à TEE. **Débouchés (28/08)** : 66 métiers extraits d'une source externe générée automatiquement (non ISPM, registre `SRC-GUIDE-FILIERES-GENERE`, statut **externe**) via `backend/scripts/preparer_candidats_metiers.py --ecrire-corpus` → fusionnés dans `backend/data/metiers.json` et `Parcours.debouches` des 16 parcours (décision produit du 28/08 : fusion telle quelle, sans relecture individuelle des 66 intitulés — même statut de provenance que les matières OCR, déjà en production). Restent incomplets : compétences, passerelles, matières de licence propres à TEE | — |
| ~~DATA-2~~ ✅ | Registre des sources | Structure (titre, URL, date de consultation, statut officiel/institutionnel/externe, données extraites, limites) — absent d'EXAM-S2 **[NOUVEAU]** — `backend/src/sources.py` + `backend/data/registre_sources.json` (7 entrées, dont une divergence de source explicitement documentée sur le sigle CAA, une expansion non confirmée pour AEE, et une source externe générée automatiquement pour les débouchés métiers) | DATA-1 |
| ~~DATA-3~~ (partiel) | Corpus structuré + provenance | Peupler `DocumentSource`/`Mention`/`Parcours` (SETUP-5) en liant chaque entrée à son entrée du registre (DATA-2) **[ADAPT]** ingestion RAG-1 d'EXAM-S2 — `source_id` posé sur tous les modèles, `verifier_provenance()` garantit qu'aucune référence n'est orpheline (testé sur les données réelles). **Incomplet** : compétences et passerelles pas encore peuplées (dépend de DATA-1) — matières et métiers/débouchés sont désormais tous les deux en production | SETUP-5, DATA-2 |
| ~~DATA-4~~ (partiel) | Questionnaire d'enquête | Rédaction et diffusion (étudiants + professionnels), lancé dès la première heure du hackathon **[NOUVEAU]** — **rédigé** : `backend/data/enquete/questionnaire.md` (version de référence, 21 questions, deux populations, consentement explicite, aucune donnée sensible) et `generer_google_form.gs`, script Apps Script qui génère le formulaire Google à l'identique depuis cette référence. **Diffusée** : 86 réponses reçues (voir DATA-5) | — |
| ~~DATA-5~~ ✅ | Registre de collecte de l'enquête | Populations visées, période, nombre de réponses reçues/retenues/écartées, texte de consentement, procédure d'anonymisation, biais constatés **[NOUVEAU]** — `backend/data/enquete/registre_collecte.md`, généré automatiquement par `backend/scripts/preparer_jeu_test_reel.py` : 86 réponses (71 étudiants, 15 professionnels), 79/86 rattachables à un parcours ou une mention connue, 62/86 à étiquette jugée fiable (satisfaction ≥ 3/5). Biais et limites (échantillon concentré sur quelques mentions, risque de ré-identification par petits effectifs) documentés dans le fichier généré, pas seulement déclarés ici | DATA-4 |
| ~~DATA-6~~ ✅ | Génération de profils synthétiques | Méthode, hypothèses, biais introduits, contrôles de cohérence documentés **[NOUVEAU]** — `backend/src/ml/donnees_synthetiques.py` + `archetypes.py` (16 archétypes ancrés sur les vraies descriptions de parcours, DATA-1) ; 800 profils dans `backend/data/ml/profils_synthetiques.json`. **Second mode ajouté (DATA-6 bis)** : génération calée sur la distribution et la complétude réellement observées dans les enquêtes — classes non équiprobables et profils volontairement incomplets, voir AUDIT-ML-2 et l'en-tête du module | SETUP-4 |
| ~~DATA-7~~ ✅ | Montage du jeu de données ML | Synthèse pour l'entraînement, réponses d'enquête gelées en fin de J1 pour validation/test **[NOUVEAU]**, recommandation explicite du sujet — synthèse (DATA-6, 800 profils, entraînement) + enquête réelle gelée (DATA-5, `backend/data/ml/jeu_test_reel.json`, 79 profils utilisables, validation/test uniquement — jamais mélangée à l'entraînement). **Limite assumée** : la seule question de niveau scolaire du questionnaire porte une note combinée « maths/info » (échelle 1-5), reportée à l'identique sur les deux matières plutôt qu'une note par matière inventée. Reste à faire : ML-7 (lancer l'évaluation elle-même sur ce jeu) | DATA-5, DATA-6 |
| ~~DATA-8~~ ✅ | Anonymisation des réponses | Retrait des identifiants directs avant livraison du jeu de données **[ADAPT]** `masquer_donnees_sensibles`/`masquer_objet` de `guardrails.py` comme base — appliqué dans `preparer_jeu_test_reel.py` : colonne de consentement retirée, horodatage réduit à la date, réponses type « Anonyme »/« Aucun » traitées comme non-réponse plutôt que comme un intitulé de poste, `masquer_objet` passé sur chaque enregistrement produit. **Limite non résolue, documentée** : avec 86 réponses, le recoupement (parcours + année + métier déclaré) reste théoriquement porteur d'un risque de ré-identification par petits effectifs, que le masquage de motifs ne supprime pas — voir `registre_collecte.md` (DATA-5) | DATA-4 |
| ~~DATA-7~~ (partiel) | Montage du jeu de données ML | Synthèse pour l'entraînement, enquête pour validation/test **[NOUVEAU]** — `backend/src/enquete_import.py` produit `reponses_orientia.json` (14 réponses exploitables, tous champs déclarés, jeu de test ML-7). **Reste** : élargir la collecte, 5 parcours sur 16 seulement étant représentés | DATA-5, DATA-6 |
...
| ~~ONTO-2~~ ✅ | Construction du graphe | Peuplement depuis le corpus structuré **[NOUVEAU]** — `backend/src/graphe.py` (`construire_graphe()`, NetworkX) ; une référence orpheline (id cité par un `Parcours` mais absent du corpus) n'aboutit jamais à une arête fantôme, elle est signalée par ONTO-4. Ajout mineur à `models.Competence` (`metiers_requis`) pour porter la relation `estRequisePour`, additif et rétrocompatible. Sur le corpus réel, le graphe compte désormais **583 nœuds et 1204 arêtes** (1106 `enseigne`, 66 `prepareA`, 16 `necessite`, 16 `appartientA`) depuis la collecte des matières (DATA-1, 27/08) et la fusion des débouchés (DATA-1, 28/08) ; compétences restent à collecter, `estRequisePour` toujours vide | ONTO-1, DATA-3 |
...
| ML-7 | Généralisation synthèse → réel | Mesurer la capacité du modèle entraîné sur profils synthétiques à généraliser aux réponses d'enquête réelles **[NOUVEAU]** — **débloqué côté données** : `backend/data/ml/jeu_test_reel.json` (DATA-7) fournit désormais 79 profils réels étiquetés. **Reste à faire** : écrire/lancer l'évaluation elle-même (`eval_ml.py` porte encore l'avertissement synthèse-seule sur tous ses chiffres, à mettre à jour une fois ce jeu branché) | ML-5, DATA-7 |
| ~~ML-7~~ ✅ | Généralisation synthèse → réel | Mesurer la capacité du modèle entraîné sur profils synthétiques à généraliser aux réponses d'enquête réelles **[NOUVEAU]** — `eval_ml.py` mesure désormais `generalisation_reelle_ml_7` : le modèle de production (`ml.outils.analyser_profil`, sans substitution — ces 79 profils réels n'ont jamais été vus à l'entraînement) appliqué au jeu réel gelé (DATA-7). **Résultat mesuré, pas favorable** : top-1 à 1,3 % (contre 99,5 % sur synthèse), **56/79 profils jugés inexploitables** par le garde-fou d'exploitabilité (ML-9) — l'enquête ne recueille que 1 à 4 matières préférées et une note combinée maths/info, très en-deçà des 5 dimensions déclarées par un profil synthétique. Écart de généralisation réel et documenté (`backend/tests/eval_results_ml.json`), pas supposé | ML-5, DATA-7 |
...
| ~~AGT-2~~ ✅ | Spécification des outils du domaine | `rechercher_formation`, `comparer_parcours`, `analyser_profil_ml`, `calculer_score_adequation`, `verifier_prerequis`, `rechercher_competences`, `identifier_debouches`, `expliquer_recommandation` **[ADAPT]** structure `OUTILS`/`TOOL_REGISTRY` — les 8 outils cités par le sujet, plus **`detecter_incoherences`** (9e, apport ONTO-4), `OUTILS` et `TOOL_REGISTRY` vérifiés cohérents par test. `verifier_prerequis` interroge désormais **le graphe** (ONTO-3), pas les données structurées. `identifier_debouches` dispose désormais de 66 métiers (DATA-1, 28/08, source externe) ; `rechercher_competences` reste data-limité : les compétences ne sont toujours pas collectées | DATA-3, ML-8, ONTO-3 |
| ~~AGT-3~~ ✅ | Validation & exécution des outils | `valider_parametres()`/`executer_outil()` **[REUSE tel quel]** | AGT-2 |
| ~~AGT-4~~ ✅ | Politique de refus / incertitude / renvoi | Remplace le concept « action sensible → validation humaine » d'EXAM-S2 par « confiance insuffisante → escalade vers un conseiller pédagogique » **[ADAPT concept]** — seuil de confiance force `escalade_conseiller` (`agent.py`). **Version complète du « code contresigne la décision »**, ajoutée dans `agent._appliquer_controles_deterministes()` : `_recouper_avec_regles_pedagogiques()` revérifie le parcours de tête via `tools.verifier_prerequis()` (requête déterministe du graphe) et force `escalade_conseiller` sur un verdict `compatible: False` établi — premier essai basé sur un marqueur textuel (`ml.hybride.MARQUEUR_REGLE_ADMISSION`) recopié dans la justification, corrigé en audit : rien ne garantissait que le modèle le reproduise mot pour mot en rédigeant sa réponse finale plutôt que de paraphraser, ce qui aurait tu le garde-fou précisément quand la consultation ML n'était pas déjà forcée ; `_verrouiller_renvoi_administration()` vide `parcours_recommandes` sur une décision `renvoi_administration` (§16 : ce n'est par définition pas un conseil pédagogique, même si un appel ML antérieur dans la même boucle en a produit un) | AGT-1, ORCH-2 |
...
| ~~AGT-6~~ ✅ | Traçabilité des réponses issues des outils structurés | **Défaut trouvé à l'évaluation post-fusion** (voir `backend/tests/eval_analyse.md`, cas EVAL-17) : `agent._appliquer_controles_deterministes()` ne conservait que les sources présentes dans le contexte RAG, filtre écrit quand le RAG était le seul chemin vers une information. **Corrigé** : `tools._fiche_parcours` et apparentés (`verifier_prerequis`, `identifier_debouches`, `expliquer_recommandation`, `rechercher_formation`) remontent désormais le `source_id` du `Parcours`/`Mention` résolu (registre DATA-2) ; `agent._source_ids_des_outils()` le collecte récursivement dans chaque résultat d'outil retourné pendant la boucle, et `_appliquer_controles_deterministes()` élargit l'ensemble des sources autorisées à ces identifiants, en plus du contexte RAG | AGT-5, ONTO-3 |
...
| ~~FE-5~~ ✅ | Migration frontend vers Next.js — front-office chatbot | `frontend-next/` **[NOUVEAU]**, `frontend/` (Streamlit) conservé tel quel, les deux coexistent. Espace candidat (`app/chat/`) reconstruit en vraie conversation (historique + profil accumulés côté client dans `sessionStorage`, renvoyés à chaque tour au même `POST /orientation/traiter` **toujours stateless côté serveur** — pas de mémoire de conversation ajoutée, cf. limite déjà documentée dans `eval_analyse.md`). Port fidèle de `front_office.py` : mention obligatoire affichée inconditionnellement, score jamais affiché si `AVERTISSEMENT_NON_EXPLOITABLE` détecté, badge « Admissibilité à vérifier » si `MARQUEUR_REGLE_ADMISSION` détecté (ces deux marqueurs sont désormais aussi exposés par `GET /health`, pour un frontend qui ne peut pas les lire par import Python), 7 scénarios de démo, profil édité explicitement (jamais déduit du texte libre du chat — SEC-4) | FE-1, FE-2, FE-3 |
| ~~FE-6~~ ✅ | Migration frontend vers Next.js — backoffice admin | `app/admin/` **[NOUVEAU]**, visuellement et structurellement séparé du chat candidat. Auth par cookie signé (`jose`, HttpOnly) plutôt que `st.session_state` local à l'onglet — même secret `ORIENTIA_ADMIN_CODE`, même repli « accès ouvert, annoncé comme tel » si absent. 5 vues portées depuis `back_office.py`, dont un tableau de bord avec **évolution des stats dans le temps** (volume, latence, confiance, répartition des actions par jour/heure — nouveau, absent du backoffice Streamlit) et un graphe de connaissances interactif (`@xyflow/react` + `dagre`, remplace le rendu DOT/graphviz qui n'a pas d'équivalent React). Nouveaux endpoints backend en lecture seule (`backend/src/admin_api.py`, `/admin/*`) exposant en HTTP ce que `back_office.py` calculait par import Python direct — **ces endpoints restent ouverts, sans authentification côté API** (l'auth vit uniquement dans Next.js, cohérent avec le fait que le prototype ne manipule aucune donnée personnelle) ; point de vigilance si l'API est un jour exposée au-delà de `localhost` | FE-1, FE-4, OBS-4 |

## 🔍 Audit externe du backend — tickets issus de la revue

Revue d'ingénierie du backend menée par un collègue sur la branche
`feature/conversation-humaine` (436 tests au vert au moment de la revue). Verdict
d'ensemble favorable sur l'ingénierie ; **le point dur est le cœur ML** : ~99,5 %
d'exactitude sur synthèse contre 1,3 % de top-1 sur l'enquête réelle, sous le
hasard (6,25 %). Diagnostic de l'audit : le modèle a appris à inverser son propre
générateur de données, pas à orienter des étudiants.

Les tickets ci-dessous reprennent la revue **sans la reformuler à la baisse**.
Sévérité reprise telle quelle : `à corriger` (impact réel probable),
`à planifier`, `à surveiller`.

### Validité prédictive du ML — par ordre d'impact

| ID | Tâche | Description | Dépendances |
|---|---|---|---|
| AUDIT-ML-1 | Reconstruire le vocabulaire depuis les vraies données | Dériver le vocabulaire des **490 matières réelles** collectées (`matieres.json`) et du texte libre de l'enquête, au lieu des 16 archétypes. Rattacher terme étudiant → matière → parcours via la relation `enseigne` du graphe, qui sait déjà que « algorithmes » ou « bases de données » mènent à IGGLIA/ESIIA/ISAIA. Remplace un mapping vers ~48 buckets flous par un rattachement structurel. **C'est le levier principal sur les 56/79 profils inexploitables** (voir AUDIT-ML-2) | ONTO-2, ML-9 |
| ~~AUDIT-ML-2~~ ✅ | Entraîner sur une distribution qui ressemble à l'enquête | **Fait et mesuré.** `src/ml/distribution_reelle.py` mesure sur les deux enquêtes réelles (101 réponses, 93 étiquetées) les effectifs par parcours **et** la complétude par champ, sans aucun chiffre codé en dur. `donnees_synthetiques.py` sait désormais générer selon un `prior` lissé (`generer_jeu_cale_sur_enquete`) et selon des **régimes de complétude** calqués sur ce qui est réellement collecté. Jeu challenger : `backend/data/ml/profils_synthetiques_realistes.json`. **Résultat mesuré sur les 79 profils réels, chemin de production identique, seules les données d'entraînement changent** : top-1 **15,2 % → 31,6 %**, top-3 **41,8 % → 63,3 %**, MRR **0,351 → 0,488**, rang médian **4 → 3**. Le jeu de référence équiprobable est **conservé intact** comme point de comparaison. **Correction apportée à l'audit** : les 56/79 profils inexploitables **ne bougent pas** (56 avant, 56 après) — le garde-fou ML-9 se prononce sur la *couverture du vocabulaire en entrée*, pas sur ce que le modèle a vu à l'entraînement. Les débloquer relève de AUDIT-ML-1 ou du seuil `ml_traits_minimum_exploitables`, pas de la distribution d'entraînement | DATA-6, DATA-7 |
| AUDIT-ML-3 | Changer la forme du problème | La classification 16 classes sur ~48 features binaires quasi séparables est le mauvais cadrage. Trois pistes : (a) **ranking par similarité** profil ↔ description de parcours avec les embeddings déjà présents pour le RAG — aucune étiquette synthétique, généralise nativement ; (b) prédire d'abord la **mention** (6 classes, mieux couverte par l'enquête) puis le parcours ; (c) un **scoreur fondé sur le graphe** comme baseline forte que le ML doit battre | AUDIT-ML-1 |
| AUDIT-ML-4 | Recalibrer sur des données réelles | La calibration isotonique est ajustée sur du synthétique quasi parfaitement séparable : elle a appris « ce modèle a presque toujours raison », faux sur de vrais candidats. La refitter sur les profils réels (ou une part tenue à l'écart), même bruités. Un intervalle large et honnête vaut mieux qu'une fausse assurance | ML-6, DATA-7 |
| AUDIT-ML-5 | Exploiter le signal de satisfaction | 62/86 répondants déclarent une satisfaction ≥ 3/5. La cible peut devenir « parcours suivi **et jugé satisfaisant** » vs signal faible/négatif — un vrai problème d'apprentissage de préférence, au lieu de « parcours suivi » seul qui inclut les réorientés | DATA-7 |
| AUDIT-ML-6 | Rendre au modèle les features qui comptent | `serie_bac` est utilisée par la règle hybride (ML-10) mais **jamais vue par le modèle** (absente de `features.noms_features()`). `activites_projets` reste à 0 % partout (constat ML-1). **Partiellement débloqué** : le générateur produit désormais `serie_bac` (62 % des profils du jeu challenger, dont une part délibérément inadmissible pour ne pas en faire une clé déterministe). **Reste à faire** : l'encoder dans le vecteur de features et réentraîner, ou retirer le code mort qui référence `activites_projets` | AUDIT-ML-2, ML-10 |
| ~~AUDIT-ML-7~~ ✅ | Aligner « modèle évalué » et « modèle servi » | **Fait.** `entrainer_et_sauvegarder()` persistait la régression **non calibrée**, alors que la production sert la version **calibrée bornée** (`ModeleBorne` enveloppant un `CalibratedClassifierCV`) — corrigé pour entraîner `entrainer_baseline_calibree`, comme `ml.outils._modele()`. Verrouillé par `test_entrainer_et_sauvegarder_persiste_le_meme_modele_que_la_production` (type) et son pendant sur les probabilités (`np.testing.assert_allclose` entre le `.joblib` rechargé et le modèle de production, sur les mêmes données) | ML-5 |
| AUDIT-ML-8 | Élargir l'enquête | 5 parcours sur 16 seulement sont représentés dans le jeu réel gelé. Une seconde vague ciblant les mentions absentes rendrait la mesure de généralisation exploitable **classe par classe** — et rendrait `distribution_reelle` (AUDIT-ML-2) nettement plus représentatif, puisqu'il se recalcule sans édition manuelle | DATA-4 |

### Constats backend hors backlog

| ID | Sévérité | Constat | Description |
|---|---|---|---|
| ~~AUDIT-C1~~ ✅ | **corrigé** | Corpus RAG non ré-indexé sur une base existante | **Fait.** L'ingestion ne se déclenchait que si `nombre_de_fragments() == 0` ; un `chroma_db/` déjà peuplé continuait de servir l'ancien corpus en ignorant `corpus_genere.json`. `rag.empreinte_corpus()` (SHA-256 sur id/titre/catégorie/source/contenu, indépendant de l'ordre) est comparée à `rag.empreinte_indexee()` (fichier `.empreinte_corpus` à côté de la base, hors métadonnées Chroma pour ne jamais toucher `hnsw:space`) via `rag.index_a_jour()`, appelée dans `api.lifespan()` à la place du seul `== 0`. `python -m src.rag --reindex` ajouté pour forcer une ré-ingestion à la demande. Vérifié en conditions réelles (empreinte effectivement écrite dans `backend/chroma_db/.empreinte_corpus`), pas seulement sous mock |
| AUDIT-B1 | à planifier | `llm_call_with_tools` combine `response_schema` et `tools` | Gemini ne garantit pas la sortie structurée quand des outils sont déclarés. L'agent ne fonctionne que grâce au repli `_valider_reponse_finale`. Fragile : un tour mêlant texte et `function_call` (avertissement « non-text parts » observé) peut casser le parsing. Séparer les deux phases, ou traiter le repli comme le chemin nominal |
| AUDIT-B2 | à planifier | Journaux JSONL non bornés et relus intégralement | `_lire_dernieres_lignes` charge tout le fichier puis coupe les N dernières ; `llm_calls.jsonl` stocke prompts et réponses complets et ne tourne jamais. Sur une démo longue, le tableau de bord admin ralentit, et une ligne partiellement écrite lève `JSONDecodeError` jusqu'au 500. Correctif : rotation + lecture par la fin tolérante aux lignes corrompues |
| AUDIT-B3 | à surveiller | Admissibilité Bac A2 : la condition de note n'est pas vérifiée | `serie_satisfait_prerequis` valide « série A2 avec note de maths ≥ 12/20 » sur le seul `\bA2\b`, sans la note. Cas limite, et c'est du conseil pas une décision — la note de maths n'est de toute façon pas collectée |
| ~~AUDIT-P1~~ ✅ | **corrigé** | Anti-injection LLM sur le chemin critique de chaque message | **Fait, avec un arbitrage explicite.** Garde-fous d'entrée et recherche documentaire s'exécutent désormais en parallèle (`orchestrator._executer_en_parallele`, `ThreadPoolExecutor` + copie du `ContextVar` de budget par tâche — indispensable, un `Context` ne s'exécute pas depuis deux threads à la fois). `retrieve_context` ne fait aucun appel réseau (embeddings ONNX locaux + Chroma) : la paralléliser avec la vérification LLM ne coûte qu'un calcul local. **Trade-off assumé et documenté en tête de module** : sur un message jugé dangereux, la recherche documentaire est désormais parfois calculée avant d'être jetée (jamais transmise à l'agent, jamais journalisée — `_finaliser` reçoit toujours `contexte=None` sur ce chemin), alors qu'avant elle n'était jamais lancée. L'effet observable (rien ne fuite) est inchangé, la lettre de l'invariant documenté a été mise à jour pour rester honnête. Verrouillé par deux tests dédiés (propagation du budget dans les threads, RAG appelée-mais-jetée sur message dangereux) + 5 exécutions consécutives sans flake |
| AUDIT-P2 | à surveiller | Graphe reconstruit à chaque requête admin | `/admin/qualite-donnees`, `/admin/corpus`, `/admin/graphe` rechargent le corpus et reconstruisent les 583 nœuds depuis le disque à chaque appel. Réutiliser celui que `tools` détient déjà |
| ~~AUDIT-P3~~ ✅ | **corrigé** | Démarrage à froid du modèle ML et de l'index | **Fait.** `ml.outils.precharger()` (nouveau) force `_modele()`, `_modele_explicatif()` et `_graphe_admission()` avant la première requête, sans jamais lever (jeu de données absent = cas normal, dépôt fraîchement cloné). `api.lifespan()` l'appelle, puis interroge `retrieve_context()` une fois pour forcer le chargement du modèle d'embedding ONNX (`.count()` seul n'y suffisait pas : c'est une opération de métadonnées, pas un calcul d'embedding). Vérifié en conditions réelles (non mocké) : lifespan complet (ingestion + double préchauffage) en 3,95 s au premier démarrage |
| AUDIT-R1 | à surveiller | État global mutable et bascules « impose puis restaure » | `tools._corpus`/`_graphe`, `ml.outils._modele_impose`/`_graphe_impose`, `llm_client._dernier_appel`. Le motif `imposer_modele_pour_evaluation()` repose sur un `try/finally` discipliné : un `finally` oublié fait fuiter l'état entre tests. `_profil_courant` est correctement un `ContextVar` — pas les autres |
| AUDIT-R2 | à surveiller | `agent.py` approche 800 lignes | ~9 fonctions de contrôle déterministe plus un très long prompt système en littéral. Encore lisible, mais le prompt mériterait un module dédié et les contrôles un `controls.py` |
| AUDIT-R3 | à surveiller | Le filtre d'historique plafonne la confiance à vie | Un tour d'historique qui déclenche le filtre mots-clés est retiré du rejeu (bien), mais la note de dégradation plafonne aussi la confiance de **toutes** les réponses suivantes. Scoper la note pour qu'elle ne touche pas la confiance |
| AUDIT-S1 | à surveiller | Endpoints `/admin/*` sans authentification | Décision documentée et acceptable pour un prototype localhost (le contrôle vit dans le frontend Next.js). À rendre bloquant avant toute exposition hors localhost — `/admin/corpus` déverse tout le corpus structuré |

### Tâches d'amélioration proposées par l'audit

| ID | Tâche | Pourquoi maintenant | Effort |
|---|---|---|---|
| AUDIT-T1 | Re-lancer `tests.eval_system` | `eval_results.json` date du 26/08, antérieur à la mémoire conversationnelle et aux derniers correctifs. Livrable exigé, actuellement périmé | faible |
| AUDIT-T2 | Régénérer les évals RAG après enrichissement du corpus | Rappel et silence tiennent à 1,00 en hybride, mais la précision stricte tombe (0,67 → 0,34). Chiffres à publier et à expliquer, pas à cacher | faible |
| AUDIT-T3 | Cas de test multi-tours dans `eval_dataset.json` | Tout le jeu est mono-tour alors que la mémoire existe désormais : référence de suivi (« cette filière »), changement de sujet, injection glissée dans l'historique | moyen |
| AUDIT-T4 | Transcript de référence pour les 9 questions-types du jury | Rejouer et figer les questions adverses connues (comparaison sourcée, « pourquoi ce parcours », « données réelles ou générées », refus de profilage) pour ne pas régresser | moyen |
| ~~AUDIT-T5~~ ✅ | Test « modèle évalué = modèle servi » | Fait avec AUDIT-ML-7 | faible |
| ~~AUDIT-T6~~ ✅ | Versionner l'index RAG (hash du corpus) | Fait avec AUDIT-C1 (fichier d'empreinte plutôt que métadonnées Chroma) | faible |
| AUDIT-T7 | Rotation des journaux + lecture tolérante | Voir AUDIT-B2 : évite le ralentissement et le 500 sur une session de démo longue | faible |
| ~~AUDIT-T8~~ ✅ | Anti-injection LLM en parallèle du RAG | Fait avec AUDIT-P1 — trade-off documenté à relire avant toute évolution du garde-fou d'entrée | moyen |
| ~~AUDIT-T9~~ ✅ | Préchauffage ML + Chroma dans `lifespan` | Fait avec AUDIT-P3 | faible |
| AUDIT-T10 | Prototype de AUDIT-ML-3 (ranking par similarité) | Aucune étiquette synthétique, généralise nativement, réutilise les embeddings du RAG. Même un résultat modeste sur l'enquête réelle serait une histoire ML bien plus solide devant le jury | élevé |

### Points relevés comme exemplaires — à ne pas casser

L'audit insiste pour que ces choix soient cités tels quels plutôt que perdus dans
la liste des correctifs :

- **« Le code contresigne la décision du modèle »**, appliqué sans exception :
  sources recoupées avec les passages réellement fournis, `outils_utilises` = appels
  réels, consultation ML forcée si le modèle recommande sans l'avoir consultée,
  escalade automatique sur contradiction règle pédagogique / ML.
- **Silence RAG calibré** (seuil 0,56 / k=5 mesurés, pas supposés) : ne rien trouver
  est un comportement voulu, et il tient.
- **Garde-fou d'exploitabilité ML** : un profil trop mince met la confiance à 0 au
  lieu d'afficher un classement trompeur — c'est ce qui fait que l'échec ML reste
  sûr.
- **Modèle hybride** (ML + règles d'admission du graphe) : rétrograde sans masquer,
  ne touche jamais aux scores, ne fait rien quand la série n'est pas déclarée.
- **Dégradation propre partout** : `traiter_demande()` ne lève jamais, budget de
  temps global par `ContextVar`, repli valide à chaque étape.
- **Honnêteté des limites** : `eval_analyse.md` nomme trois défauts trouvés et
  corrigés, et la généralisation synthèse → réel est **mesurée**, pas supposée.
