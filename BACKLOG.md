...
| AGT-7 | Cohérence entre la prose de l'agent et le classement du modèle | **Défaut trouvé en vérifiant ML-9, reproduit 2 fois sur 2** : sur un profil informatique, `parcours_recommandes[0]` vaut IGGLIA (0,54) mais l'explication en prose annonce « le modèle recommande en priorité ESIIA » (0,11). L'agent narre à partir des passages RAG plutôt que du classement de l'outil. Le barème note explicitement « cohérence entre le modèle ML et la réponse finale ». **Correctif** : contrôle déterministe dans `agent._appliquer_controles_deterministes()` — si l'explication nomme un parcours qui n'est pas le mieux classé, annexer une note `[Contrôle automatique]` rappelant le classement réel (même mécanisme que pour les sources retirées et la consultation ML forcée) | AGT-5, ML-8 |
