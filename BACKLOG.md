...
| ~~DATA-4~~ (partiel) | Questionnaire d'enquête | Rédaction et diffusion (étudiants + professionnels), lancé dès la première heure du hackathon **[NOUVEAU]** — **rédigé** : `backend/data/enquete/questionnaire.md` (version de référence, 21 questions, deux populations, consentement explicite, aucune donnée sensible) et `generer_google_form.gs`, script Apps Script qui génère le formulaire Google à l'identique depuis cette référence. **Diffusée** : 15 réponses reçues, 14 exploitables (voir DATA-5) | — |
| ~~DATA-5~~ ✅ | Registre de collecte de l'enquête | Populations visées, période, nombre de réponses reçues/retenues/écartées, texte de consentement, procédure d'anonymisation, biais constatés **[NOUVEAU]** — `backend/data/enquete/registre_collecte.json`, dont les chiffres sont vérifiés contre les données par test (`test_les_chiffres_du_registre_correspondent_aux_donnees`) : **15 réponses reçues, 14 retenues, 1 écartée** (formation hors ISPM), 14 étudiants et 1 professionnel, collectées du 26 au 27 août 2026. Six limites nommées plutôt que déclarées : volume (±22 points d'intervalle de confiance), 5 parcours sur 16 représentés, une seule réponse professionnelle, auto-sélection par les réseaux de l'équipe, fenêtre de moins de 24 h, et collecte antérieure à la correction du saut de page | DATA-4 |
...
| ~~ML-7~~ ✅ | Généralisation synthèse → réel | Mesurer la capacité du modèle entraîné sur profils synthétiques à généraliser aux réponses d'enquête réelles **[NOUVEAU]** — mesuré sur **nos 14 profils exploitables** via `tests/eval_ml.py` (`generalisation_reelle_ml_7`), avec le modèle réellement servi (`ml.outils.analyser_profil`), sans substitution : **top-1 = 0,643**, MRR 0,677, rang médian de la bonne classe = 1, aucun profil jugé inexploitable. **Le repère honnête n'est pas le hasard (6,2 %) mais la classe majoritaire** : l'échantillon compte 9 IGGLIA sur 14, et une règle répondant toujours IGGLIA obtiendrait 0,643 — le modèle l'égale sans la dépasser. `top_3 = top_1` : quand il se trompe, le bon parcours n'est pas non plus dans les trois premiers. À cette taille deux profils valent 14 points, donc aucune conclusion forte. Cette mesure a révélé et fait corriger l'effondrement de la calibration isotonique (4 profils sur 14 rendaient une distribution uniforme, top-1 0,50 avant correction) | ML-5, DATA-7 |
